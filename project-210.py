...
def number_of_vehicle():
    bot_vehicles = world.get_actors().filter('vehicle.*')
    ...
    if len(bot_vehicles) > 1:
        ...
        for x in :#complete the loop by adding bot car variable
            if  != world.id:#compare with id
                get_distance_of_bot_vehicles.((distance(x.get_location()), x))#append list
